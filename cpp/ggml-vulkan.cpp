...
typedef lm_ggml_fp16_t half;
struct lm_ggml_kompute_context {
    bool hasH2DAll = false;
    std::vector<lm_ggml_vk_memory> buffers;
...
lm_ggml_kompute_context *s_kompute_context = nullptr;
...
static bool lm_ggml_vk_checkPhysicalDeviceFeatures(vk::PhysicalDevice physicalDevice) {
...
static std::string lm_ggml_vk_getVendorName(uint32_t vendorID) {
    switch (vendorID) {
        case 0x10DE:
            return "nvidia";
        case 0x1002:
...
std::vector<lm_ggml_vk_device> lm_ggml_vk_available_devices(size_t memoryRequired) {
    std::vector<lm_ggml_vk_device> results;
    ...
    for (uint32_t i = 0; i < deviceCount; i++) {
        ...
        if (!lm_ggml_vk_checkPhysicalDeviceFeatures(physicalDevices.at(i)))
            continue;
        ...
        d.vendor = lm_ggml_vk_getVendorName(properties.vendorID);
        results.push_back(d);
    }

    std::stable_sort(results.begin(), results.end(),
        [](const lm_ggml_vk_device& lhs, const lm_ggml_vk_device& rhs) -> bool {
...
static void lm_ggml_vk_filterByVendor(std::vector<lm_ggml_vk_device>& devices, const std::string& targetVendor) {
    devices.erase(
        std::remove_if(devices.begin(), devices.end(),
            [&targetVendor](const lm_ggml_vk_device& device) {
...
static void lm_ggml_vk_filterByName(std::vector<lm_ggml_vk_device>& devices, const std::string& targetName) {
    devices.erase(
        std::remove_if(devices.begin(), devices.end(),
            [&targetName](const lm_ggml_vk_device& device) {
...
bool lm_ggml_vk_init_device(size_t memoryRequired, const std::string &device) {
    ...
    std::vector<lm_ggml_vk_device> devices = lm_ggml_vk_available_devices(memoryRequired);
    if (device == "gpu") {
        if (devices.size() != 0)
            return lm_ggml_vk_init_device(devices.front());
    } else if (device == "amd" || device == "nvidia" || device == "intel") {
        lm_ggml_vk_filterByVendor(devices, device);
        if (devices.size() != 0)
            return lm_ggml_vk_init_device(devices.front());
    } else {
        lm_ggml_vk_filterByName(devices, device);
        if (devices.size() != 0)
            return lm_ggml_vk_init_device(devices.front());
    }

    return lm_ggml_vk_has_device();
}

bool lm_ggml_vk_init_device(const lm_ggml_vk_device &device) {
    return lm_ggml_vk_init_device(device.index);
}
...
lm_ggml_vk_device lm_ggml_vk_current_device() {
    if (!komputeManager()->hasDevice())
        return lm_ggml_vk_device();

    std::vector<lm_ggml_vk_device> devices = lm_ggml_vk_available_devices(0);
    lm_ggml_vk_filterByName(devices, komputeManager()->physicalDevice()->getProperties().deviceName);
    return devices.front();
}

lm_ggml_kompute_context *lm_ggml_vk_init() {
    s_kompute_context = new lm_ggml_kompute_context;
...
static
void lm_ggml_vk_allocate_descriptor_pool(struct lm_ggml_kompute_context * ctx, size_t size) {
...
static
void lm_ggml_vk_free_descriptor_pool(struct lm_ggml_kompute_context * ctx) {
    if (ctx->pool) {
...
static
vk::Buffer *lm_ggml_vk_allocate_buffer(size_t size) {
...
static
vk::DeviceMemory *lm_ggml_vk_allocate(size_t size, vk::MemoryPropertyFlags flags, vk::MemoryRequirements requirements, bool *isHostVisible) {
...
static void lm_ggml_vk_h2d_buffer(const lm_ggml_vk_memory &memory) {
...
static void lm_ggml_vk_d2h_buffer(const lm_ggml_vk_memory &memory) {
    if (memory.stagingBuffer)
...
lm_ggml_vk_memory lm_ggml_vk_allocate(size_t size) {
    ...
    {
        memory.primaryBuffer = lm_ggml_vk_allocate_buffer(size);
        ...
        memory.primaryMemory = lm_ggml_vk_allocate(size, memoryPropertyFlags, memoryRequirements, &isHostVisible);
    ...
    if (!isHostVisible) {
        memory.stagingBuffer = lm_ggml_vk_allocate_buffer(size);
        ...
        memory.stagingMemory = lm_ggml_vk_allocate(size, memoryPropertyFlags, memoryRequirements, &isHostVisible);
...
static
decltype(lm_ggml_kompute_context::buffers)::iterator lm_ggml_vk_find_tensor(struct lm_ggml_kompute_context * ctx, struct lm_ggml_tensor * t, uint64_t & offset) {
    for (auto it = ctx->buffers.begin(); ; it++) {
        ...
        if (it->data <= t->data &&
                reinterpret_cast<intptr_t>(it->data) + it->size >= (reinterpret_cast<intptr_t>(t->data) + lm_ggml_nbytes(t))) {
...
static
const std::shared_ptr<kp::Tensor> lm_ggml_vk_get_tensor(struct lm_ggml_kompute_context * ctx, struct lm_ggml_tensor * t, uint32_t *alignedOffset) {
    uint64_t originalOffset = 0;
    auto res = lm_ggml_vk_find_tensor(ctx, t, originalOffset);
    ...
    const size_t nelements = lm_ggml_nelements(t);
    size_t nbytes = lm_ggml_nbytes(t);

    size_t vulkanOffset = lm_ggml_vk_aligned_offset(originalOffset);
    if (alignedOffset) {
...
void lm_ggml_vk_h2d_tensor(struct lm_ggml_kompute_context * ctx, struct lm_ggml_tensor * t) {
    const auto res = lm_ggml_vk_get_tensor(ctx, t, nullptr);
    LM_GGML_ASSERT(res);
...
uint32_t safe_divide(uint32_t a, uint32_t b) {
    ...
    if ((a % b) != 0) {
        ...
        LM_GGML_ASSERT(!"safe_divide result would've had remainder");
    }
    return a / b;
}

void lm_ggml_vk_add(kp::Sequence& seq,
...
void lm_ggml_vk_addrow(kp::Sequence& seq,
                 const std::shared_ptr<kp::Tensor>& inA,
                 const std::shared_ptr<kp::Tensor>& inB,
                 const std::shared_ptr<kp::Tensor>& out,
                 uint32_t inAOff, uint32_t inBOff, uint32_t outOff,
...
void lm_ggml_vk_mul(kp::Sequence& seq,
                    const std::shared_ptr<kp::Tensor>& inA,
                    const std::shared_ptr<kp::Tensor>& inB,
                    const std::shared_ptr<kp::Tensor>& out,
                    uint32_t inAOff, uint32_t inBOff, uint32_t outOff,
...
void lm_ggml_vk_mulrow(kp::Sequence& seq,
                 const std::shared_ptr<kp::Tensor>& inA,
                 const std::shared_ptr<kp::Tensor>& inB,
                 const std::shared_ptr<kp::Tensor>& out,
                 uint32_t inAOff, uint32_t inBOff, uint32_t outOff,
...
void lm_ggml_vk_scale(kp::Sequence& seq,
...
void lm_ggml_vk_xxlu(const std::vector<uint32_t>& spirv, kp::Sequence& seq,
                  const std::shared_ptr<kp::Tensor>& in,
                  const std::shared_ptr<kp::Tensor>& out,
                  uint32_t inOff, uint32_t outOff,
                  uint32_t size) {
...
template <typename... Args>
void lm_ggml_vk_gelu(Args&&... args) {
...
void lm_ggml_vk_soft_max(kp::Sequence& seq,
                      const std::shared_ptr<kp::Tensor>& in,
                      const std::shared_ptr<kp::Tensor>& out,
                      uint32_t inOff, uint32_t outOff,
...
void lm_ggml_vk_norm_(const std::vector<uint32_t>& spirv, kp::Sequence& seq,
                   const std::shared_ptr<kp::Tensor>& in,
                   const std::shared_ptr<kp::Tensor>& out,
                   uint32_t inOff, uint32_t outOff,
                   int32_t ne00, int32_t nb01,
                   int32_t nrows, float epsilon) {
    LM_GGML_ASSERT(nb01%sizeof(float) == 0);
    LM_GGML_ASSERT(ne00%sizeof(float) == 0);
...
template <typename... Args>
void lm_ggml_vk_norm(Args&&... args) {
    const static auto spirv = getSpirvShader(kp::shader_data::op_norm_comp_spv,
        kp::shader_data::op_norm_comp_spv_len);

    lm_ggml_vk_norm_(spirv, std::forward<Args>(args)...);
}

template <typename... Args>
void lm_ggml_vk_rms_norm(Args&&... args) {
...
void lm_ggml_vk_diag_mask_inf(kp::Sequence& seq,
                           const std::shared_ptr<kp::Tensor>& in,
                           const std::shared_ptr<kp::Tensor>& out,
                           uint32_t inOff, uint32_t outOff,
                           uint32_t n_past,
...
void lm_ggml_vk_mul_mat_f16(kp::Sequence& seq,
                         const std::shared_ptr<kp::Tensor>& inA,
                         const std::shared_ptr<kp::Tensor>& inB,
                         const std::shared_ptr<kp::Tensor>& out,
                         uint32_t inAOff, uint32_t inBOff, uint32_t outOff,
    ...
    std::shared_ptr<kp::Algorithm> s_algo = nullptr;
    if (!komputeManager()->hasAlgorithm(__func__)) {
        const uint32_t local_x = lm_ggml_vk_current_device().subgroupSize * 2;
...
void lm_ggml_vk_mul_mat_q8_0(kp::Sequence& seq,
                         const std::shared_ptr<kp::Tensor>& inA,
                         const std::shared_ptr<kp::Tensor>& inB,
                         const std::shared_ptr<kp::Tensor>& out,
                         uint32_t inAOff, uint32_t inBOff, uint32_t outOff,
    ...
    std::shared_ptr<kp::Algorithm> s_algo = nullptr;
    if (!komputeManager()->hasAlgorithm(__func__)) {
        const uint32_t local_x = lm_ggml_vk_current_device().subgroupSize;
...
void lm_ggml_vk_mul_mat_mat_f32(kp::Sequence& seq,
                         const std::shared_ptr<kp::Tensor>& inA,
                         const std::shared_ptr<kp::Tensor>& inB,
                         const std::shared_ptr<kp::Tensor>& out,
                         uint32_t inAOff, uint32_t inBOff, uint32_t outOff,
...
void lm_ggml_vk_mul_mat_q4_x(const std::vector<uint32_t>& spirv, uint32_t block_size, kp::Sequence& seq,
                          const std::shared_ptr<kp::Tensor>& inA,
                          const std::shared_ptr<kp::Tensor>& inB,
                          const std::shared_ptr<kp::Tensor>& out,
                          uint32_t inAOff, uint32_t inBOff, uint32_t outOff,
...
template <typename... Args>
void lm_ggml_vk_mul_mat_q4_0(Args&&... args) {
    ...
    lm_ggml_vk_mul_mat_q4_x(spirv, 1/*We access blocks unaligned*/, std::forward<Args>(args)...);
}

template <typename... Args>
void lm_ggml_vk_mul_mat_q4_1(Args&&... args) {
...
void lm_ggml_vk_mul_mat_q6_k(kp::Sequence& seq,
                          const std::shared_ptr<kp::Tensor>& inA,
                          const std::shared_ptr<kp::Tensor>& inB,
                          const std::shared_ptr<kp::Tensor>& out,
                          uint32_t inAOff, uint32_t inBOff, uint32_t outOff,
...
void lm_ggml_vk_get_rows(const std::vector<uint32_t>& spirv,
                      ...
                      uint32_t size) {
    LM_GGML_ASSERT(nb01%element_size == 0);
    LM_GGML_ASSERT(nb1%sizeof(float) == 0);
    if (qk) LM_GGML_ASSERT(ne00%qk == 0);
...
template <typename... Args>
void lm_ggml_vk_get_rows_f16(Args&&... args) {
    const static auto spirv = getSpirvShader(kp::shader_data::op_getrows_f16_comp_spv,
        kp::shader_data::op_getrows_f16_comp_spv_len);

    lm_ggml_vk_get_rows(spirv, sizeof(half), 0, std::forward<Args>(args)...);
}

template <typename... Args>
void lm_ggml_vk_get_rows_q4_0(Args&&... args) {
    ...
    lm_ggml_vk_get_rows(spirv, 1/*We access blocks unaligned*/, QK4_0, std::forward<Args>(args)...);
}

template <typename... Args>
void lm_ggml_vk_get_rows_q4_1(Args&&... args) {
    ...
    lm_ggml_vk_get_rows(spirv, 1/*We access blocks unaligned*/, QK4_1, std::forward<Args>(args)...);
}

template <typename... Args>
void lm_ggml_vk_get_rows_q6_k(Args&&... args) {
    ...
    lm_ggml_vk_get_rows(spirv, 1/*We access blocks unaligned*/, QK_NL, std::forward<Args>(args)...);
}

void lm_ggml_vk_rope(kp::Sequence& seq,
                  const std::shared_ptr<kp::Tensor>& in,
                  const std::shared_ptr<kp::Tensor>& out,
                  uint32_t inOff, uint32_t outOff,
    ...
    LM_GGML_ASSERT(nb03%sizeof(float) == 0);
    LM_GGML_ASSERT(nb02%sizeof(float) == 0);
    LM_GGML_ASSERT(nb01%sizeof(float) == 0);
    LM_GGML_ASSERT(nb00%sizeof(float) == 0);
    LM_GGML_ASSERT(nb3%sizeof(float) == 0);
    LM_GGML_ASSERT(nb2%sizeof(float) == 0);
    LM_GGML_ASSERT(nb1%sizeof(float) == 0);
    LM_GGML_ASSERT(nb0%sizeof(float) == 0);
...
void lm_ggml_vk_cpy(const std::vector<uint32_t>& spirv,
                 kp::Sequence& seq,
                 const std::shared_ptr<kp::Tensor>& in,
                 const std::shared_ptr<kp::Tensor>& out,
                 uint32_t inOff, uint32_t outOff,
...
void lm_ggml_vk_graph_compute(struct lm_ggml_kompute_context * ctx, struct lm_ggml_cgraph * gf) {
    ...
    lm_ggml_vk_allocate_descriptor_pool(ctx, gf->n_nodes);
    ...
    for (int seq_idx = 0; seq_idx < n_seq; ++seq_idx) {
        ...
        for (int i = node_start; i < node_end; ++i) {
            ...
            LM_GGML_ASSERT(dst->data != nullptr);
...
//            const int32_t ne3 = dst ? dst->ne[3] : 0;
            ...
            uint32_t off_src1 = 0;
            uint32_t off_dst = 0;
            const std::shared_ptr<kp::Tensor>& id_src0 = src0 ? lm_ggml_vk_get_tensor(ctx, src0, &off_src0) : nullTensor;
            const std::shared_ptr<kp::Tensor>& id_src1 = src1 ? lm_ggml_vk_get_tensor(ctx, src1, &off_src1) : nullTensor;
            const std::shared_ptr<kp::Tensor>& id_dst  = dst ? lm_ggml_vk_get_tensor(ctx, dst, &off_dst)  : nullTensor;
            ...
            switch (dst->op) {
                ...
                case LM_GGML_OP_ADD:
                    {
                        if (lm_ggml_nelements(src1) == ne10) {
                            // src1 is a row
                            lm_ggml_vk_addrow(seq, id_src0, id_src1, id_dst, off_src0, off_src1, off_dst, lm_ggml_nelements(dst)/4, ne00);
                        } else {
                            lm_ggml_vk_add(seq, id_src0, id_src1, id_dst,  off_src0, off_src1, off_dst, lm_ggml_nelements(dst)/4);
                        }
                    } break;
                case LM_GGML_OP_MUL:
                    {
                        if (lm_ggml_nelements(src1) == ne10) {
                            // src1 is a row
                            lm_ggml_vk_mulrow(seq, id_src0, id_src1, id_dst, off_src0, off_src1, off_dst, lm_ggml_nelements(dst)/4, ne00);
                        } else {
                            lm_ggml_vk_mul(seq, id_src0, id_src1, id_dst, off_src0, off_src1, off_dst, lm_ggml_nelements(dst)/4);
                        }
                    } break;
                case LM_GGML_OP_SCALE:
                    {
                        const float scale = *(const float *) src1->data;
                        lm_ggml_vk_scale(seq, id_src0, id_dst, off_src0, off_dst, lm_ggml_nelements(dst)/8, scale);
                    } break;
                case LM_GGML_OP_UNARY:
                    switch (lm_ggml_get_unary_op(gf->nodes[i])) {
                        case LM_GGML_UNARY_OP_SILU:
                            {
                                lm_ggml_vk_silu(seq, id_src0, id_dst, off_src0, off_dst, lm_ggml_nelements(dst)/4);
                            } break;
                        case LM_GGML_UNARY_OP_RELU:
                            {
                                lm_ggml_vk_relu(seq, id_src0, id_dst, off_src0, off_dst, lm_ggml_nelements(dst)/4);
                            } break;
                        case LM_GGML_UNARY_OP_GELU:
                            {
                                lm_ggml_vk_gelu(seq, id_src0, id_dst, off_src0, off_dst, lm_ggml_nelements(dst)/8);
                            } break;
                        default:
                            {
                ...
                case LM_GGML_OP_SOFT_MAX:
                    {
                        lm_ggml_vk_soft_max(seq, id_src0, id_dst, off_src0, off_dst, ne00, ne01, ne02, ne03);
                    } break;
                case LM_GGML_OP_DIAG_MASK_INF:
                    {
                        const int n_past = ((int32_t *)(dst->op_params))[0];
                        lm_ggml_vk_diag_mask_inf(seq, id_src0, id_dst, off_src0, off_dst, n_past, ne00, ne01, ne02);
                    } break;
                case LM_GGML_OP_NORM:
                    {
                        float eps;
                        memcpy(&eps, dst->op_params, sizeof(float));
                        lm_ggml_vk_norm(seq, id_src0, id_dst, off_src0, off_dst, ne00, nb01, lm_ggml_nrows(src0), eps);
                    } break;
                case LM_GGML_OP_RMS_NORM:
                    {
                        float eps;
                        memcpy(&eps, dst->op_params, sizeof(float));
                        lm_ggml_vk_rms_norm(seq, id_src0, id_dst, off_src0, off_dst, ne00, nb01, lm_ggml_nrows(src0), eps);
                    } break;
                case LM_GGML_OP_MUL_MAT:
                    {
                        if (src1t != LM_GGML_TYPE_F32) {
                            fprintf(stderr, "%s: %s: Unsupported src1 type: %u/%u\n", __func__, lm_ggml_op_name(dst->op), src0t, src1t);
                        ...
                        switch (src0t) {
                            case LM_GGML_TYPE_F32:
                                lm_ggml_vk_mul_mat_mat_f32(seq,
                            ...
                            case LM_GGML_TYPE_F16:
                                lm_ggml_vk_mul_mat_f16(seq, id_src0, id_src1, id_dst, off_src0, off_src1, off_dst, ne00, ne01, ne02, nb01, nb02, ne11, ne12, nb11, nb12, ne0, ne1);
                                break;
                            case LM_GGML_TYPE_Q8_0:
                                lm_ggml_vk_mul_mat_q8_0(seq, id_src0, id_src1, id_dst, off_src0, off_src1, off_dst, ne00, ne01, nb01, nb02, ne11, ne12, nb11, nb12, ne0, ne1);
                                break;
                            case LM_GGML_TYPE_Q4_0:
                                lm_ggml_vk_mul_mat_q4_0(seq, id_src0, id_src1, id_dst, off_src0, off_src1, off_dst, ne00, ne10, ne0, ne1, ne01, ne11, ne12, ne02);
                                break;
                            case LM_GGML_TYPE_Q4_1:
                                lm_ggml_vk_mul_mat_q4_1(seq, id_src0, id_src1, id_dst, off_src0, off_src1, off_dst, ne00, ne10, ne0, ne1, ne01, ne11, ne12, ne02);
                                break;
                            case LM_GGML_TYPE_Q6_K:
                                lm_ggml_vk_mul_mat_q6_k(seq, id_src0, id_src1, id_dst, off_src0, off_src1, off_dst, ne00, ne10, ne0, ne1, ne01, ne11, ne12, ne02);
                                break;
                            default: {
                ...
                case LM_GGML_OP_GET_ROWS:
                    {
                        if (src0t == LM_GGML_TYPE_F16) {
                            lm_ggml_vk_get_rows_f16(seq, id_src0, id_src1, id_dst, off_src0, off_src1, off_dst, ne00, nb01, nb1, lm_ggml_nelements(src1));
                        } else if (src0t == LM_GGML_TYPE_Q4_0) {
                            lm_ggml_vk_get_rows_q4_0(seq, id_src0, id_src1, id_dst, off_src0, off_src1, off_dst, ne00, nb01, nb1, lm_ggml_nelements(src1));
                        } else if (src0t == LM_GGML_TYPE_Q4_1) {
                            lm_ggml_vk_get_rows_q4_1(seq, id_src0, id_src1, id_dst, off_src0, off_src1, off_dst, ne00, nb01, nb1, lm_ggml_nelements(src1));
                        } else if (src0t == LM_GGML_TYPE_Q6_K) {
                            lm_ggml_vk_get_rows_q6_k(seq, id_src0, id_src1, id_dst, off_src0, off_src1, off_dst, ne00, nb01, nb1, lm_ggml_nelements(src1));
                        } else {
                            fprintf(stderr, "%s: %s: Unsupported quantization: %u\n", __func__, lm_ggml_op_name(dst->op), src0t);
                            goto not_implemented;
                        }
                    } break;
                case LM_GGML_OP_ROPE:
                    {
                        ...
                        lm_ggml_vk_rope(seq, id_src0, id_dst, off_src0, off_dst, n_past, n_dims, mode, freq_base, freq_scale, ne01, ne02, ne03, nb00, nb01, nb02, nb03, ne0, nb0, nb1, nb2, nb3);
                    } break;
                case LM_GGML_OP_DUP:
                case LM_GGML_OP_CPY:
                case LM_GGML_OP_CONT:
                    {
                        switch (src0t) {
                            case LM_GGML_TYPE_F32:
                                {
                                    switch (dstt) {
                                        case LM_GGML_TYPE_F16: lm_ggml_vk_cpy_f32_f16(seq, id_src0, id_dst, off_src0, off_dst, ne00, ne01, ne02, ne03, nb00, nb01, nb02, nb03, ne0, ne1, ne2, nb0, nb1, nb2, nb3); break;
                                        case LM_GGML_TYPE_F32: lm_ggml_vk_cpy_f32_f32(seq, id_src0, id_dst, off_src0, off_dst, ne00, ne01, ne02, ne03, nb00, nb01, nb02, nb03, ne0, ne1, ne2, nb0, nb1, nb2, nb3); break;
                            ...
                            case LM_GGML_TYPE_F16:
                                {
                                    switch (dstt) {
                                        case LM_GGML_TYPE_F16: lm_ggml_vk_cpy_f16_f16(seq, id_src0, id_dst, off_src0, off_dst, ne00, ne01, ne02, ne03, nb00, nb01, nb02, nb03, ne0, ne1, ne2, nb0, nb1, nb2, nb3); break;
                                        case LM_GGML_TYPE_F32: lm_ggml_vk_cpy_f16_f32(seq, id_src0, id_dst, off_src0, off_dst, ne00, ne01, ne02, ne03, nb00, nb01, nb02, nb03, ne0, ne1, ne2, nb0, nb1, nb2, nb3); break;
                                    default: goto not_implemented;
                                } break;
                            default: goto not_implemented;
                            }
    ...
    lm_ggml_vk_free_descriptor_pool(ctx);
}
