...
static void lm_ggml_graph_compute_helper(std::vector<uint8_t> & buf, lm_ggml_cgraph * graph, int n_threads) {
    struct lm_ggml_cplan plan = lm_ggml_graph_plan(graph, n_threads);
    ...
    for (int i = 0; i < graph->n_nodes; i++) {
        lm_ggml_vk_preallocate_buffers_graph(graph->nodes[i]);
    }
    lm_ggml_vk_preallocate_buffers();

    for (int i = 0; i < graph->n_nodes; i++) {
        lm_ggml_vk_build_graph(graph->nodes[i]);
    }
#endif

    lm_ggml_graph_compute(graph, &plan);

#ifdef LM_GGML_USE_VULKAN
    lm_ggml_vk_graph_cleanup();
#endif
}
...
#ifdef LM_GGML_USE_CUBLAS
#   define llama_host_malloc(n)  lm_ggml_cuda_host_malloc(n)
#   define llama_host_free(data) lm_ggml_cuda_host_free(data)
...
#elif LM_GGML_USE_VULKAN
#   define llama_host_malloc(n)  lm_ggml_vk_host_malloc(n)
#   define llama_host_free(data) lm_ggml_vk_host_free(data)
#else
...
typedef void (*offload_func_t)(struct lm_ggml_tensor * tensor);

static void llama_nop(struct lm_ggml_tensor * tensor) { // don't offload by default
    (void) tensor;
}
...
struct llama_layer {
    ...
    // ff
    struct lm_ggml_tensor * w1; // ffn_gate
    struct lm_ggml_tensor * w2; // ffn_down
    struct lm_ggml_tensor * w3; // ffn_up

    // ff bias
    struct lm_ggml_tensor * b2; // ffn_down
...
#ifdef LM_GGML_USE_CUBLAS
        lm_ggml_cuda_free_data(k);
        lm_ggml_cuda_free_data(v);
...
struct llama_model {
    ...
    struct lm_ggml_tensor * pos_embeddings;
    ...
    ~llama_model() {
        ...
        for (size_t i = 0; i < tensors_by_name.size(); ++i) {
            lm_ggml_cuda_free_data(tensors_by_name[i].second);
        }
        lm_ggml_cuda_free_scratch();
#elif defined(LM_GGML_USE_CLBLAST)
        for (size_t i = 0; i < tensors_by_name.size(); ++i) {
            lm_ggml_cl_free_data(tensors_by_name[i].second);
        }
#elif defined(LM_GGML_USE_VULKAN)
        for (size_t i = 0; i < tensors_by_name.size(); ++i) {
            lm_ggml_vk_free_data(tensors_by_name[i].second);
        }
#endif
    }
};
...
struct llama_context {
    ...
    // reusable buffer for `struct lm_ggml_graph_plan.work_data`
    ...
    lm_ggml_allocr * alloc = NULL;

#ifdef LM_GGML_USE_METAL
    lm_ggml_metal_context * ctx_metal = NULL;
#endif
...
static bool llama_kv_cache_init(
    ...
    cache.buf.resize(2u*n_elements*lm_ggml_type_size(wtype) + 2u*lm_ggml_tensor_overhead());
    ...
    cache.k = lm_ggml_new_tensor_1d(cache.ctx, wtype, n_elements);
    cache.v = lm_ggml_new_tensor_1d(cache.ctx, wtype, n_elements);
    lm_ggml_set_name(cache.k, "cache_k");
    lm_ggml_set_name(cache.v, "cache_v");
...
struct llama_model_loader {
    ...
    llama_model_loader(const std::string & fname, bool use_mmap) : file(fname.c_str(), "rb") {
        ...
        // TODO: make optional
        {
            ...
            for (int i = 0; i < n_tensors; i++) {
                ...
                LLAMA_LOG_INFO("%s: - tensor %4d: %32s %-8s [ %s ]\n", __func__, i, name, lm_ggml_type_name(meta->type), llama_format_tensor_shape(meta).c_str());
            }
    ...
    struct lm_ggml_tensor * get_tensor_meta(int i) const {
        return lm_ggml_get_tensor(ctx_meta, get_tensor_name(i));
    }
    ...
    void calc_sizes(size_t & ctx_size_p, size_t & mmapped_size_p) const {
        ...
        for (int i = 0; i < n_tensors; i++) {
            struct lm_ggml_tensor * meta = get_tensor_meta(i);
            ctx_size_p += sizeof(struct lm_ggml_tensor) + LM_GGML_OBJECT_SIZE;
            (use_mmap ? mmapped_size_p : ctx_size_p) += lm_ggml_nbytes_pad(meta);
        }
    }

    struct lm_ggml_tensor * create_tensor_for(struct lm_ggml_context * ctx, struct lm_ggml_tensor * meta, lm_ggml_backend_type backend) {
        if (backend != LM_GGML_BACKEND_CPU) {
            lm_ggml_set_no_alloc(ctx, true);
        }

        struct lm_ggml_tensor * tensor = lm_ggml_dup_tensor(ctx, meta);
        tensor->backend = backend; // TODO: lm_ggml_set_backend
        lm_ggml_set_name(tensor, lm_ggml_get_name(meta));

        if (backend != LM_GGML_BACKEND_CPU) {
            lm_ggml_set_no_alloc(ctx, use_mmap);
        }
    ...
    struct lm_ggml_tensor * create_tensor(struct lm_ggml_context * ctx, const std::string & name, const std::vector<int64_t> & ne, lm_ggml_backend_type backend) {
    ...
    void load_all_data(struct lm_ggml_context * ctx, llama_progress_callback progress_callback, void * progress_callback_user_data, llama_mlock * lmlock) {
        size_t size_data = 0;
        size_t size_lock = 0;
        ...
        if (use_mmap) {
            mapping.reset(new llama_mmap(&file, size_pref, lm_ggml_is_numa()));
            if (lmlock) {
        ...
        for (int i = 0; i < gguf_get_n_tensors(ctx_gguf); i++) {
            ...
            switch (cur->backend) {
                case LM_GGML_BACKEND_CPU:
                    if (use_mmap && lmlock) {
                        size_lock += lm_ggml_nbytes(cur);
                ...
                case LM_GGML_BACKEND_GPU_SPLIT:
                    // old code:
                    //lm_ggml_cuda_transform_tensor(lt.data, lt.lm_ggml_tensor);

                    // TODO: test if this works !!
                    lm_ggml_cuda_transform_tensor(cur->data, cur);
                    if (!use_mmap) {
                        free(cur->data);
                    }
                    break;
#elif defined(LM_GGML_USE_CLBLAST)
                case LM_GGML_BACKEND_GPU:
                    lm_ggml_cl_transform_tensor(cur->data, cur);
                    if (!use_mmap) {
                        free(cur->data);
                    }
                    break;
#elif defined(LM_GGML_USE_VULKAN)
                case LM_GGML_BACKEND_GPU:
                    lm_ggml_vk_transform_tensor(cur->data, cur);
                    if (!use_mmap) {
                        free(cur->data);
                    }
                    break;
...
static void llm_load_tensors(
    ...
    // create the ggml context
    {
        ...
        if (!model.ctx) {
            throw std::runtime_error(format("lm_ggml_init() failed"));
        }
    }
...
#define LLAMA_BACKEND_OFFLOAD       LM_GGML_BACKEND_GPU
#define LLAMA_BACKEND_OFFLOAD_SPLIT LM_GGML_BACKEND_GPU
#elif defined(LM_GGML_USE_VULKAN)
    fprintf(stderr, "%s: using Vulkan for GPU acceleration\n", __func__);
#define LLAMA_BACKEND_OFFLOAD       LM_GGML_BACKEND_GPU
#define LLAMA_BACKEND_OFFLOAD_SPLIT LM_GGML_BACKEND_GPU
#else
#define LLAMA_BACKEND_OFFLOAD       LM_GGML_BACKEND_CPU
#define LLAMA_BACKEND_OFFLOAD_SPLIT LM_GGML_BACKEND_CPU
#endif
            ...
            case LLM_ARCH_REFACT:
                {
                    ...
                    // output
                    {
                        lm_ggml_backend_type backend_norm;
                        lm_ggml_backend_type backend_output;
                        ...
                        if (n_gpu_layers > int(n_layer)) {
                            ...
                            backend_norm   = LM_GGML_BACKEND_CPU;
                            backend_output = LM_GGML_BACKEND_CPU;
                        }

                        model.output_norm = ml.create_tensor(ctx, tn(LLM_TENSOR_OUTPUT_NORM, "weight"), {n_embd},          backend_norm);
                        model.output      = ml.create_tensor(ctx, tn(LLM_TENSOR_OUTPUT,      "weight"), {n_embd, n_vocab}, backend_output);

                        if (backend_norm == LM_GGML_BACKEND_GPU) {
                            vram_weights += lm_ggml_nbytes(model.output_norm);
                        }
                        if (backend_output == LM_GGML_BACKEND_GPU_SPLIT) {
                            vram_weights += lm_ggml_nbytes(model.output);
                        }
                    }
                    ...
                    for (uint32_t i = 0; i < n_layer; ++i) {
                        const lm_ggml_backend_type backend = int(i) < i_gpu_start ? LM_GGML_BACKEND_CPU : LLAMA_BACKEND_OFFLOAD; // NOLINT
                        const lm_ggml_backend_type backend_split = int(i) < i_gpu_start ? LM_GGML_BACKEND_CPU : LLAMA_BACKEND_OFFLOAD_SPLIT; // NOLINT
                        ...
                        if (backend == LM_GGML_BACKEND_GPU) {
                            vram_weights +=
                                lm_ggml_nbytes(layer.attn_norm) + lm_ggml_nbytes(layer.wq) + lm_ggml_nbytes(layer.wk)       +
                                lm_ggml_nbytes(layer.wv)        + lm_ggml_nbytes(layer.wo) + lm_ggml_nbytes(layer.ffn_norm) +
                                lm_ggml_nbytes(layer.w1)        + lm_ggml_nbytes(layer.w2) + lm_ggml_nbytes(layer.w3);
                        }
                    }
                } break;
            case LLM_ARCH_BAICHUAN:
            ...
            case LLM_ARCH_FALCON:
                {
                    ...
                    // output
                    {
                        ...
                        if (backend_norm == LM_GGML_BACKEND_GPU) {
                            vram_weights += lm_ggml_nbytes(model.output_norm);
                            vram_weights += lm_ggml_nbytes(model.output_norm_b);
                        }
                        if (backend_output == LM_GGML_BACKEND_GPU_SPLIT) {
                            vram_weights += lm_ggml_nbytes(model.output);
                        }
                    ...
                    for (uint32_t i = 0; i < n_layer; ++i) {
                        ...
                        if (gguf_find_tensor(ml.ctx_gguf, tn(LLM_TENSOR_ATTN_NORM_2, "weight", i).c_str()) >= 0) {
                            ...
                            if (backend == LM_GGML_BACKEND_GPU) {
                                vram_weights += lm_ggml_nbytes(layer.attn_norm_2);
                                vram_weights += lm_ggml_nbytes(layer.attn_norm_2_b);
                            }
                        }
                        ...
                        if (backend == LM_GGML_BACKEND_GPU) {
                            vram_weights +=
                                lm_ggml_nbytes(layer.attn_norm) + lm_ggml_nbytes(layer.attn_norm_b) +
                                lm_ggml_nbytes(layer.wqkv)      + lm_ggml_nbytes(layer.wo)          +
                                lm_ggml_nbytes(layer.w2)        + lm_ggml_nbytes(layer.w3);
                        }
                    }
                } break;
            case LLM_ARCH_STARCODER:
                {
                    model.tok_embeddings = ml.create_tensor(ctx, tn(LLM_TENSOR_TOKEN_EMBD, "weight"), {n_embd, n_vocab}, LM_GGML_BACKEND_CPU);
                    model.pos_embeddings = ml.create_tensor(ctx, tn(LLM_TENSOR_POS_EMBD, "weight"), {n_embd, hparams.n_ctx_train}, LM_GGML_BACKEND_CPU);
                    ...
                    for (uint32_t i = 0; i < n_layer; ++i) {
                        ...
                        if (backend == LM_GGML_BACKEND_GPU) {
                            vram_weights +=
                                lm_ggml_nbytes(layer.attn_norm) + lm_ggml_nbytes(layer.attn_norm_b) +
                                lm_ggml_nbytes(layer.wqkv)      + lm_ggml_nbytes(layer.bqkv)        +
                                lm_ggml_nbytes(layer.wo)        + lm_ggml_nbytes(layer.bo)          +
                                lm_ggml_nbytes(layer.ffn_norm)  + lm_ggml_nbytes(layer.ffn_norm_b)  +
                                lm_ggml_nbytes(layer.w2)        + lm_ggml_nbytes(layer.b2)          +
                                lm_ggml_nbytes(layer.w3)        + lm_ggml_nbytes(layer.b3);
                        }
                    }
                } break;
            case LLM_ARCH_PERSIMMON:
                {
                    ...
                    model.layers.resize(n_layer);
                    for (uint32_t i = 0; i < n_layer; ++i) {
                        const lm_ggml_backend_type backend = int(i) < i_gpu_start ? LM_GGML_BACKEND_CPU : LLAMA_BACKEND_OFFLOAD;
                        const lm_ggml_backend_type backend_split = int(i) < i_gpu_start ? LM_GGML_BACKEND_CPU : LLAMA_BACKEND_OFFLOAD_SPLIT;
                        auto & layer = model.layers[i];
                        layer.attn_norm   = ml.create_tensor(ctx, tn(LLM_TENSOR_ATTN_NORM, "weight", i), {n_embd}, backend);
                        layer.attn_norm_b = ml.create_tensor(ctx, tn(LLM_TENSOR_ATTN_NORM, "bias", i),   {n_embd}, backend);
                        layer.wqkv = ml.create_tensor(ctx, tn(LLM_TENSOR_ATTN_QKV, "weight", i), {n_embd, n_embd + 2*n_embd_gqa}, backend_split);
            ...
            case LLM_ARCH_BLOOM:
                {
                    ...
                    model.tok_embeddings = ml.create_tensor(ctx, tn(LLM_TENSOR_TOKEN_EMBD,      "weight"), {n_embd, n_vocab}, LM_GGML_BACKEND_CPU);
                    model.tok_norm       = ml.create_tensor(ctx, tn(LLM_TENSOR_TOKEN_EMBD_NORM, "weight"), {n_embd},          LM_GGML_BACKEND_CPU);
                    model.tok_norm_b     = ml.create_tensor(ctx, tn(LLM_TENSOR_TOKEN_EMBD_NORM, "bias"),   {n_embd},          LM_GGML_BACKEND_CPU);
            ...
            case LLM_ARCH_MPT:
                {
                    ...
                    for (uint32_t i = 0; i < n_layer; ++i) {
                        ...
                        if (backend == LM_GGML_BACKEND_GPU) {
                            vram_weights +=
                                lm_ggml_nbytes(layer.attn_norm) +
                                lm_ggml_nbytes(layer.wqkv)      +
                                lm_ggml_nbytes(layer.wo)        +
                                lm_ggml_nbytes(layer.ffn_norm)  +
                                lm_ggml_nbytes(layer.w2)        +
                                lm_ggml_nbytes(layer.w3);
                        }
                    }
                } break;
            default:
...
#if defined(LM_GGML_USE_CUBLAS) || defined(LM_GGML_USE_CLBLAST) || defined(LM_GGML_USE_VULKAN)
...
#elif defined(LM_GGML_USE_CLBLAST) || defined(LM_GGML_USE_VULKAN)
...
#else
        (void) n_gpu_layers;
#endif // defined(LM_GGML_USE_CUBLAS) || defined(LM_GGML_USE_CLBLAST) || defined(LM_GGML_USE_VULKAN)
    }

    // populate `tensors_by_name`
    for (int i = 0; i < ml.n_tensors; ++i) {
        struct lm_ggml_tensor * cur = lm_ggml_get_tensor(ctx, ml.get_tensor_name(i));
...
static struct lm_ggml_cgraph * llm_build_baichaun(
    ...
    for (int il = 0; il < n_layer; ++il) {
        ...
        // self-attention
        {
            ...
            struct lm_ggml_tensor * KQ_masked;
            struct lm_ggml_tensor * KQ_scaled_alibi;
...
static struct lm_ggml_cgraph * llm_build_falcon(
    ...
    for (int il = 0; il < n_layer; ++il) {
        ...
        {
            attn_norm = lm_ggml_norm(ctx0, inpL, norm_eps);
            offload_func(attn_norm);

            attn_norm = lm_ggml_add(ctx0,
                    lm_ggml_mul(ctx0, attn_norm, model.layers[il].attn_norm),
                    model.layers[il].attn_norm_b);
            offload_func(attn_norm->src[0]);
            offload_func(attn_norm);

            if (model.layers[il].attn_norm_2) { // Falcon-40B
                cur = lm_ggml_norm(ctx0, inpL, norm_eps);
                offload_func(cur);

                cur = lm_ggml_add(ctx0,
                        lm_ggml_mul(ctx0, cur, model.layers[il].attn_norm_2),
            ...
            const size_t wsize = lm_ggml_type_size(cur->type);
        ...
        // feed forward
        {
            struct lm_ggml_tensor * inpFF = attn_norm;

            cur = lm_ggml_mul_mat(ctx0, model.layers[il].w3, inpFF);
            offload_func(cur);

            cur = lm_ggml_gelu(ctx0, cur);
            offload_func(cur);
            cur = lm_ggml_mul_mat(ctx0, model.layers[il].w2, cur);
            offload_func(cur);
        }
    ...
    // norm
    {
        ...
        cur = lm_ggml_add(ctx0,
                lm_ggml_mul(ctx0, cur, model.output_norm),
                model.output_norm_b);
        lm_ggml_set_name(cur, "result_norm");
    }
...
static struct lm_ggml_cgraph * llm_build_starcoder(
    ...
    if (batch.token) {
        ...
        token = lm_ggml_get_rows(ctx0, model.tok_embeddings, inp_tokens);
    } else {
#ifdef LM_GGML_USE_MPI
        LM_GGML_ASSERT(false && "not implemented");
#endif
    ...
    for (int il = 0; il < n_layer; ++il) {
        {
            // Norm
            cur = lm_ggml_norm(ctx0, inpL, norm_eps);
            cur = lm_ggml_add(ctx0, lm_ggml_mul(ctx0, cur, model.layers[il].attn_norm), model.layers[il].attn_norm_b);
        }
        ...
        // Projection
        cur = lm_ggml_add(ctx0, lm_ggml_mul_mat(ctx0, model.layers[il].wo, cur), model.layers[il].bo);

        // Add the input
        cur = lm_ggml_add(ctx0, cur, inpL);

        struct lm_ggml_tensor * inpFF = cur;

        // FF
        {
            // Norm
            {
                cur = lm_ggml_norm(ctx0, inpFF, norm_eps);
                cur = lm_ggml_add(ctx0, lm_ggml_mul(ctx0, cur, model.layers[il].ffn_norm), model.layers[il].ffn_norm_b);
            }

            cur = lm_ggml_add(ctx0, lm_ggml_mul_mat(ctx0, model.layers[il].w3, cur), model.layers[il].b3);

            // GELU activation
            cur = lm_ggml_gelu(ctx0, cur);

            // Projection
            cur = lm_ggml_add(ctx0, lm_ggml_mul_mat(ctx0, model.layers[il].w2, cur), model.layers[il].b2);
        }

        inpL = lm_ggml_add(ctx0, cur, inpFF);
    }

    // Output Norm
    {
        cur = lm_ggml_norm(ctx0, inpL, norm_eps);
        cur = lm_ggml_add(ctx0, lm_ggml_mul(ctx0, cur, model.output_norm), model.output_norm_b);
    }
    lm_ggml_set_name(cur, "result_norm");
...
static struct lm_ggml_cgraph * llm_build_persimmon(
         llama_context & lctx,
     const llama_batch & batch) {
    const auto & model = lctx.model;
    const auto & hparams = model.hparams;
            ...
            struct lm_ggml_tensor * tmp =
                    ...
                    lm_ggml_rope_custom_inplace(ctx0,
                        lm_ggml_view_3d(ctx0, kv_self.k,
                            ...
                            lm_ggml_element_size(kv_self.k)*n_embd_gqa,
                            lm_ggml_element_size(kv_self.k)*n_embd_head,
                            lm_ggml_element_size(kv_self.k)*(n_embd_head*n_ctx*il)
                        ),
    ...
    for (int il=0; il < n_layer; ++il) {
        ...
        offload_func_t offload_func = llama_nop;
        {
            ...
            cur = lm_ggml_mul(ctx0, cur, model.layers[il].attn_norm);
            offload_func(cur);
            cur = lm_ggml_add(ctx0, cur, model.layers[il].attn_norm_b);
            offload_func(cur);
            lm_ggml_format_name(cur, "input_layernorm_%d", il);
        }
        // self attention
        {
            cur = lm_ggml_mul_mat(ctx0, model.layers[il].wqkv, cur);
            ...
            lm_ggml_set_name(cur, format("qkv_%d", il).c_str());
            ...
            lm_ggml_format_name(tmpqkv_perm, "tmpqkv_perm_%d", il);
            ...
            offload_func_kq(tmpk);
            // Q/K Layernorm
            tmpq = lm_ggml_norm(ctx0, tmpq, norm_eps);
            offload_func_kq(tmpq);
            tmpq =  lm_ggml_mul(ctx0, tmpq, model.layers[il].attn_q_norm);
            offload_func_kq(tmpq);
            tmpq =  lm_ggml_add(ctx0, tmpq, model.layers[il].attn_q_norm_b);
            offload_func_kq(tmpq);

            tmpk = lm_ggml_norm(ctx0, tmpk, norm_eps);
            offload_func_v(tmpk);
            tmpk =  lm_ggml_mul(ctx0, tmpk, model.layers[il].attn_k_norm);
            offload_func_v(tmpk);
            tmpk =  lm_ggml_add(ctx0, tmpk, model.layers[il].attn_k_norm_b);
            offload_func_v(tmpk);
            ...
            lm_ggml_format_name(qrot, "qrot_%d", il);
            ...
            lm_ggml_format_name(krot, "krot_%d", il);
            ...
            lm_ggml_format_name(qpass, "qpass_%d", il);
            ...
            lm_ggml_format_name(kpass, "kpass_%d", il);
            ...
            struct lm_ggml_tensor * Q = lm_ggml_cont(ctx0, lm_ggml_permute(ctx0, Qcur, 1, 2, 0, 3));
            offload_func_kq(Q);
            ...
            offload_func_kq(Kcur);
            {
                ...
                struct lm_ggml_tensor * k = lm_ggml_view_1d(
                    ctx0, kv_self.k, n_tokens*n_embd_gqa,
                    (lm_ggml_element_size(kv_self.k)*n_embd_gqa)*(il*n_ctx + kv_head)
                );
                offload_func_kq(k);
                lm_ggml_set_name(k, "k");
            ...
            struct lm_ggml_tensor * K = lm_ggml_view_3d(ctx0, kv_self.k,
                    n_embd_head, n_kv, n_head_kv,
                    lm_ggml_element_size(kv_self.k)*n_embd_gqa,
                    lm_ggml_element_size(kv_self.k)*n_embd_head,
                    lm_ggml_element_size(kv_self.k)*n_embd_gqa*n_ctx*il);
        ...
        struct lm_ggml_tensor * inpFF = lm_ggml_add(ctx0, residual, cur);
        offload_func(inpFF);
        lm_ggml_set_name(inpFF, "inpFF");
        {
            // MLP
            {
                ...
                offload_func(cur);
                cur = lm_ggml_add(ctx0,
                    lm_ggml_mul(ctx0, cur, model.layers[il].ffn_norm),
                    model.layers[il].ffn_norm_b
                );
                lm_ggml_set_name(cur, "ffn_norm");
                offload_func(cur);
            }
            cur = lm_ggml_mul_mat(ctx0, model.layers[il].w3, cur);
            offload_func(cur);

            cur = lm_ggml_add(ctx0, cur, model.layers[il].b3);
            offload_func(cur);
            lm_ggml_set_name(cur, "result_ffn_up");

            cur = lm_ggml_sqr(ctx0, lm_ggml_relu(ctx0, cur));
            lm_ggml_set_name(cur, "result_ffn_act");
            offload_func(cur);
            offload_func(cur->src[0]);
            ...
            cur = lm_ggml_add(ctx0,
                ...
                model.layers[il].b2);
            offload_func(cur);
            lm_ggml_set_name(cur, "outFF");
        }
        cur = lm_ggml_add(ctx0, cur, inpFF);
        offload_func(cur);
        lm_ggml_set_name(cur, "inpFF_+_outFF");
        inpL = cur;
    }
    cur = inpL;
    {
        ...
        cur = lm_ggml_add(ctx0, cur, model.output_norm_b);
        // offload_func_nr(cur);
...
static struct lm_ggml_cgraph * llm_build_bloom(
         llama_context & lctx,
     const llama_batch & batch) {
    const auto & model   = lctx.model;
    const auto & hparams = model.hparams;
    ...
    // norm
    {
        inpL = lm_ggml_norm(ctx0, token, norm_eps);
        inpL = lm_ggml_add(ctx0, lm_ggml_mul(ctx0, inpL, model.tok_norm), model.tok_norm_b);
    }
    ...
    for (int il = 0; il < n_layer; ++il) {
...

        {
            ...
            struct lm_ggml_tensor * KQ_scaled_alibi = lm_ggml_alibi(ctx0, KQ_scaled, /*n_past*/ kv_head, n_head, 8);
            lm_ggml_set_name(KQ_scaled_alibi, "KQ_scaled_alibi");
...
static struct lm_ggml_cgraph * llm_build_mpt(
         llama_context & lctx,
     const llama_batch & batch) {
    const auto & model   = lctx.model;
    const auto & hparams = model.hparams;
    ...
    for (int il = 0; il < n_layer; ++il) {
        ...
        // TODO: refactor into common function (shared with LLaMA)
        {
            ...
            attn_norm = lm_ggml_mul(ctx0, attn_norm, model.layers[il].attn_norm);
            offload_func(attn_norm);
            ...
            if (clamp_kqv > 0.0f) {
                cur = lm_ggml_clamp(ctx0, cur, -clamp_kqv, clamp_kqv);
                offload_func_kq(cur);
            }

            const size_t wsize = lm_ggml_type_size(cur->type);

            struct lm_ggml_tensor * Qcur = lm_ggml_view_3d(
                ctx0, cur, n_embd_head, n_head, n_tokens,
                wsize * n_embd_head,
                wsize * n_embd_head * (n_head + 2 * n_head_kv),
                0);
            ...
            struct lm_ggml_tensor * Kcur = lm_ggml_view_3d(
                ctx0, cur, n_embd_head, n_head_kv, n_tokens,
                wsize * n_embd_head,
                wsize * n_embd_head * (n_head + 2 * n_head_kv),
                wsize * n_embd_head *  n_head);
            ...
            // TODO: replace with lm_ggml_add()
            struct lm_ggml_tensor * KQ_scaled_alibi =
                lm_ggml_alibi(ctx0, KQ_scaled, 0, n_head, max_alibi_bias);
        ...
        // feed forward
        {
            // Norm
            {
                cur = lm_ggml_norm(ctx0, attn_out, norm_eps);
                offload_func(cur);
...
static int llama_decode_internal(
    ...
    LM_GGML_ASSERT(strcmp(res->name,        "result_output") == 0);
    LM_GGML_ASSERT(strcmp(embeddings->name, "result_norm")   == 0);
    ...
    for (int i = 0; i < gf->n_nodes; i++) {
        lm_ggml_tensor * node = gf->nodes[i];
        if (node->backend == LM_GGML_BACKEND_GPU && node->extra == NULL) {
            lm_ggml_cuda_assign_scratch_offset(node, (char*)node->data - (char *) lctx.buf_alloc.data);
        }
    }
    ...
    if (!lctx.embedding.empty()) {
        embeddings->backend = LM_GGML_BACKEND_CPU;
    }
...
#ifdef LM_GGML_USE_METAL
    if (lctx.ctx_metal) {
        lm_ggml_metal_set_n_cb     (lctx.ctx_metal, n_threads);
        lm_ggml_metal_graph_compute(lctx.ctx_metal, gf);
    } else {
        lm_ggml_graph_compute_helper(lctx.work_buffer, gf, n_threads);
    }
#else
    lm_ggml_graph_compute_helper(lctx.work_buffer, gf, n_threads);
#endif
...
static void llama_convert_tensor_internal(
    ...
    auto block_size = tensor->type == LM_GGML_TYPE_F16 ? 1 : (size_t)lm_ggml_blck_size(tensor->type);
    auto block_size_bytes = lm_ggml_type_size(tensor->type);
...
#ifdef LM_GGML_USE_K_QUANTS
static lm_ggml_type get_k_quant_type(
    lm_ggml_type new_type, const lm_ggml_tensor * tensor, const llama_model & model, llama_ftype ftype, int * i_attention_wv,
    ...
    if (name == tn(LLM_TENSOR_OUTPUT, "weight")) {
        ...
        if      (ftype == LLAMA_FTYPE_MOSTLY_Q2_K) new_type = LM_GGML_TYPE_Q3_K;
            ...
            new_type = *i_attention_wv < 2 ? LM_GGML_TYPE_Q5_K : LM_GGML_TYPE_Q4_K;
        }
        else if (ftype == LLAMA_FTYPE_MOSTLY_Q3_K_L) new_type = LM_GGML_TYPE_Q5_K;
        else if ((ftype == LLAMA_FTYPE_MOSTLY_Q4_K_M || ftype == LLAMA_FTYPE_MOSTLY_Q5_K_M) &&
                use_more_bits(*i_attention_wv, n_attention_wv)) new_type = LM_GGML_TYPE_Q6_K;
        else if (ftype == LLAMA_FTYPE_MOSTLY_Q4_K_S && *i_attention_wv < 4) new_type = LM_GGML_TYPE_Q5_K;
        else if (QK_K == 64 && (ftype == LLAMA_FTYPE_MOSTLY_Q4_K_S || ftype == LLAMA_FTYPE_MOSTLY_Q3_K_S) &&
                (*i_attention_wv < n_attention_wv/8 || *i_attention_wv >= 7*n_attention_wv/8)) new_type = LM_GGML_TYPE_Q6_K;
        ...
        if      (ftype == LLAMA_FTYPE_MOSTLY_Q2_K) new_type = LM_GGML_TYPE_Q3_K;
            ...
            new_type = *i_feed_forward_w2 < 2 ? LM_GGML_TYPE_Q5_K
                     : model.arch != LLM_ARCH_FALCON || use_more_bits(*i_feed_forward_w2, n_feed_forward_w2) ? LM_GGML_TYPE_Q4_K
                     : LM_GGML_TYPE_Q3_K;
        }
        else if (ftype == LLAMA_FTYPE_MOSTLY_Q3_K_L) {
            new_type = model.arch == LLM_ARCH_FALCON ? LM_GGML_TYPE_Q4_K : LM_GGML_TYPE_Q5_K;
        }
        else if (ftype == LLAMA_FTYPE_MOSTLY_Q4_K_M) {
            if (model.arch == LLM_ARCH_FALCON) {
                new_type = *i_feed_forward_w2 < 2 ? LM_GGML_TYPE_Q6_K :
                           use_more_bits(*i_feed_forward_w2, n_feed_forward_w2) ? LM_GGML_TYPE_Q5_K : LM_GGML_TYPE_Q4_K;
            } else {
                if (use_more_bits(*i_feed_forward_w2, n_feed_forward_w2)) new_type = LM_GGML_TYPE_Q6_K;
            }
        }
        else if (ftype == LLAMA_FTYPE_MOSTLY_Q5_K_M && use_more_bits(*i_feed_forward_w2, n_feed_forward_w2)) new_type = LM_GGML_TYPE_Q6_K;
        ...
        if (ftype == LLAMA_FTYPE_MOSTLY_Q3_K_M || ftype == LLAMA_FTYPE_MOSTLY_Q3_K_L) new_type = LM_GGML_TYPE_Q4_K;
    ...
    if (new_type == LM_GGML_TYPE_Q2_K || new_type == LM_GGML_TYPE_Q3_K || new_type == LM_GGML_TYPE_Q4_K ||
        new_type == LM_GGML_TYPE_Q5_K || new_type == LM_GGML_TYPE_Q6_K) {
        int nx = tensor->ne[0];
        int ny = tensor->ne[1];
        if (nx % QK_K != 0) {
    ...
    if (convert_incompatible_tensor) {
        if (name == tn(LLM_TENSOR_OUTPUT, "weight")) {
            new_type = LM_GGML_TYPE_F16; //fall back to F16 instead of just failing.
            LLAMA_LOG_WARN("F16 will be used for this tensor instead.\n");
        } else if (name == tn(LLM_TENSOR_TOKEN_EMBD, "weight")) {
            new_type = LM_GGML_TYPE_Q4_0; //fall back to Q4_0 instead of just failing.
...
static void llama_model_quantize_internal(const std::string & fname_inp, const std::string & fname_out, const llama_model_quantize_params * params) {
    lm_ggml_type quantized_type;
    llama_ftype ftype = params->ftype;

    switch (params->ftype) {
        case LLAMA_FTYPE_MOSTLY_Q4_0: quantized_type = LM_GGML_TYPE_Q4_0; break;
        case LLAMA_FTYPE_MOSTLY_Q4_1: quantized_type = LM_GGML_TYPE_Q4_1; break;
        case LLAMA_FTYPE_MOSTLY_Q5_0: quantized_type = LM_GGML_TYPE_Q5_0; break;
        case LLAMA_FTYPE_MOSTLY_Q5_1: quantized_type = LM_GGML_TYPE_Q5_1; break;
        case LLAMA_FTYPE_MOSTLY_Q8_0: quantized_type = LM_GGML_TYPE_Q8_0; break;
        case LLAMA_FTYPE_MOSTLY_F16:  quantized_type = LM_GGML_TYPE_F16;  break;
        case LLAMA_FTYPE_ALL_F32:     quantized_type = LM_GGML_TYPE_F32;  break;

#ifdef LM_GGML_USE_K_QUANTS
        // K-quants
        case LLAMA_FTYPE_MOSTLY_Q2_K:   quantized_type = LM_GGML_TYPE_Q2_K; break;
        case LLAMA_FTYPE_MOSTLY_Q3_K_S:
        case LLAMA_FTYPE_MOSTLY_Q3_K_M:
        case LLAMA_FTYPE_MOSTLY_Q3_K_L: quantized_type = LM_GGML_TYPE_Q3_K; break;
        case LLAMA_FTYPE_MOSTLY_Q4_K_S:
        case LLAMA_FTYPE_MOSTLY_Q4_K_M: quantized_type = LM_GGML_TYPE_Q4_K; break;
        case LLAMA_FTYPE_MOSTLY_Q5_K_S:
        case LLAMA_FTYPE_MOSTLY_Q5_K_M: quantized_type = LM_GGML_TYPE_Q5_K; break;
        case LLAMA_FTYPE_MOSTLY_Q6_K:   quantized_type = LM_GGML_TYPE_Q6_K; break;
#endif
    ...
    for (int i = 0; i < ml.n_tensors; ++i) {
        ...
        if (!quantize) {
            ...
            LLAMA_LOG_INFO("quantizing to %s .. ", lm_ggml_type_name(new_type));
            fflush(stdout);
            ...
            if (nthread_use < 2) {
                new_size = lm_ggml_quantize_chunk(new_type, f32_data, new_data, 0, nelements, hist_cur.data());
            } else {
                size_t counter = 0;
                new_size = 0;
                auto compute = [&mutex, &counter, &hist_cur, &new_size, new_type, f32_data, new_data, nelements]() {
                    ...
                    while (true) {
                        ...
                        local_size += lm_ggml_quantize_chunk(new_type, f32_data, new_data, first, last - first, local_hist.data());
                    }
                };
            ...
            LLAMA_LOG_INFO("size = %8.2f MB -> %8.2f MB | hist: ", lm_ggml_nbytes(tensor)/1024.0/1024.0, new_size/1024.0/1024.0);
            int64_t tot_count = 0;
...
static int llama_apply_lora_from_file_internal(
    ...
    lm_ggml_context * lora_ctx = lm_ggml_init(params);
    std::unordered_map<std::string, struct lm_ggml_tensor *> lora_tensors;

    // create a name -> tensor map of the model to accelerate lookups
    std::unordered_map<std::string, struct lm_ggml_tensor*> model_tensors;
    ...
    if (path_base_model) {
        ...
        lm_ggml_init_params base_params;
        base_params.mem_size   = base_buf.size();
        base_params.mem_buffer = base_buf.data();
        base_params.no_alloc   = ml->use_mmap;

        base_ctx = lm_ggml_init(base_params);
    ...
    while (true) {
        ...
        lm_ggml_tensor * lora_tensor;
        if (n_dims == 2) {
            lora_tensor = lm_ggml_new_tensor_2d(lora_ctx, wtype, ne[0], ne[1]);
        }
        else {
            LLAMA_LOG_ERROR("%s: unsupported tensor dimension %d\n", __func__, n_dims);
            return 1;
        }
        lm_ggml_set_name(lora_tensor, "lora_tensor");

        // load tensor data
        size_t offset = fin.tellg();
        size_t tensor_data_size = lm_ggml_nbytes(lora_tensor);
        offset = (offset + 31) & -32;
        fin.seekg(offset);
        ...
        if (lora_tensors.find(base_name + ".loraA") != lora_tensors.end() &&
            ...
            lm_ggml_tensor * dest_t = model_tensors[base_name];

            offload_func_t offload_func = llama_nop;
            offload_func_t offload_func_force_inplace = llama_nop;

#ifdef LM_GGML_USE_CUBLAS
            if (dest_t->backend == LM_GGML_BACKEND_GPU || dest_t->backend == LM_GGML_BACKEND_GPU_SPLIT) {
                if (dest_t->type != LM_GGML_TYPE_F16) {
                    throw std::runtime_error(format(
                        "%s: error: the simultaneous use of LoRAs and GPU acceleration is only supported for f16 models", __func__));
                }
                offload_func = lm_ggml_cuda_assign_buffers;
                offload_func_force_inplace = lm_ggml_cuda_assign_buffers_force_inplace;
            }
#endif // LM_GGML_USE_CUBLAS
            ...
            if (ml) {
                ...
                base_t = ml->create_tensor(base_ctx, base_name, { (uint32_t)dest_t->ne[0], (uint32_t)dest_t->ne[1] }, LM_GGML_BACKEND_CPU);
                ml->load_data_for(base_t);
            } else {
                base_t = dest_t;
            }

            if (lm_ggml_is_quantized(base_t->type)) {
                if (!warned) {
            ...
            lm_ggml_tensor * loraA = lora_tensors[base_name + ".loraA"];
            LM_GGML_ASSERT(loraA->type == LM_GGML_TYPE_F32);
            lm_ggml_set_name(loraA, "loraA");

            lm_ggml_tensor * loraB = lora_tensors[base_name + ".loraB"];
            LM_GGML_ASSERT(loraB->type == LM_GGML_TYPE_F32);
            lm_ggml_set_name(loraB, "loraB");
            ...
            if (scaling != 1.0f) {
                lm_ggml_tensor * scale_tensor = lm_ggml_new_f32(lora_ctx, scaling);
                lm_ggml_set_name(scale_tensor, "scale_tensor");

                BA = lm_ggml_scale_inplace(lora_ctx, BA, scale_tensor);
                offload_func(BA);
            ...
            lm_ggml_tensor * r;
            if (base_t == dest_t) {
                r = lm_ggml_add_inplace(lora_ctx, dest_t, BA);
                offload_func_force_inplace(r);
                lm_ggml_set_name(r, "r_add_inplace");
            }
            else {
                r = lm_ggml_add(lora_ctx, base_t, BA);
                offload_func(r);
                lm_ggml_set_name(r, "r_add");

                r = lm_ggml_cpy(lora_ctx, r, dest_t);
                offload_func(r);
...
struct llama_context * llama_new_context_with_model(
    ...
    if (!hparams.vocab_only) {
        ...
        {
            const size_t memory_size = lm_ggml_nbytes(ctx->kv_self.k) + lm_ggml_nbytes(ctx->kv_self.v);
...

        {
            ...
            ctx->buf_compute.resize(lm_ggml_tensor_overhead()*LM_GGML_MAX_NODES + lm_ggml_graph_overhead());
...
static void llama_copy_state_data_internal(struct llama_context * ctx, llama_data_context * data_ctx) {
    ...
    // copy kv cache
    {
        ...
        if (kv_buf_size) {
            ...
            lm_ggml_build_forward_expand(&gf, lm_ggml_cpy(cpy_ctx, v3d, vout3d));
            lm_ggml_graph_compute_helper(ctx->work_buffer, &gf, /*n_threads*/ 1);
...
size_t llama_set_state_data(struct llama_context * ctx, uint8_t * src) {
    ...
    // set logits
    {
        ...
        LM_GGML_ASSERT(ctx->logits.capacity() == logits_cap);
    ...
    // set embeddings
    {
        ...
        LM_GGML_ASSERT(ctx->embedding.capacity() == embedding_size);
    ...
    // set kv cache
    {
        ...
        if (kv_buf_size) {
            ...
            lm_ggml_build_forward_expand(&gf, lm_ggml_cpy(cpy_ctx, vin3d, v3d));
            lm_ggml_graph_compute_helper(ctx->work_buffer, &gf, /*n_threads*/ 1);
