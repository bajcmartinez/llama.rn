...
#define CL_CHECK(err)                                               \
    do {                                                            \
        cl_int err_ = (err);                                        \
        if (err_ != CL_SUCCESS) {                                   \
            fprintf(stderr, "lm_ggml_opencl: %s error %d at %s:%d\n",  \
...
static cl_program build_program_from_source(cl_context ctx, cl_device_id dev, const char* program_buffer) {
    ...
    if(err < 0) {
        ...
        fprintf(stderr, "lm_ggml_opencl: kernel compile error:\n\n%s\n", program_log);
        free(program_log);
        exit(1);
    }
...
void lm_ggml_cl_init(void) {
    ...
    LOGI("lm_ggml_opencl: found %d platforms and %d devices.", n_platforms, n_devices);
    ...
    if (n_devices == 0) {
        fprintf(stderr, "lm_ggml_opencl: could find any OpenCL devices.\n");
        exit(1);
    }
    ...
    if (user_platform_number != -1 && user_device_number != -1) {
        ...
        if ((unsigned)user_device_number >= platform->n_devices) {
            fprintf(stderr, "lm_ggml_opencl: invalid device number %d\n", user_device_number);
            exit(1);
        }
        ...
        if (user_platform_number == -1 && user_platform_string != NULL && user_platform_string[0] != 0) {
            ...
            if (user_platform_number == -1) {
                fprintf(stderr, "lm_ggml_opencl: no platform matching '%s' was found.\n", user_platform_string);
                exit(1);
            }
        }
        if (user_platform_number != -1) {
            ...
            if (n_selected_devices == 0) {
                fprintf(stderr, "lm_ggml_opencl: selected platform '%s' does not have any devices.\n", p->name);
                exit(1);
            }
        }
        ...
        if (user_device_number == -1 && user_device_string != NULL && user_device_string[0] != 0) {
            ...
            if (user_device_number == -1) {
                fprintf(stderr, "lm_ggml_opencl: no device matching '%s' was found.\n", user_device_string);
                exit(1);
            }
        }
        if (user_device_number != -1) {
        ...
        LM_GGML_ASSERT(n_selected_devices > 0);
    ...
    fprintf(stderr, "lm_ggml_opencl: selecting platform: '%s'\n", default_device->platform->name);
    fprintf(stderr, "lm_ggml_opencl: selecting device: '%s'\n", default_device->name);
    if (default_device->type != CL_DEVICE_TYPE_GPU) {
        fprintf(stderr, "lm_ggml_opencl: warning, not a GPU: '%s'.\n", default_device->name);
    }
    ...
    fprintf(stderr, "lm_ggml_opencl: device FP16 support: %s\n", fp16_support ? "true" : "false");
...
static cl_kernel* lm_ggml_get_to_fp32_cl(lm_ggml_type type) {
    switch (type) {
...
static size_t lm_ggml_cl_global_denom(lm_ggml_type type) {
    switch (type) {
        case LM_GGML_TYPE_Q4_0:
        case LM_GGML_TYPE_Q4_1:
        case LM_GGML_TYPE_Q5_0:
...
static size_t lm_ggml_cl_local_size(lm_ggml_type type) {
    switch (type) {
        case LM_GGML_TYPE_Q4_0:
        case LM_GGML_TYPE_Q4_1:
        case LM_GGML_TYPE_Q5_0:
...
static cl_kernel* lm_ggml_get_dequantize_mul_mat_vec_cl(lm_ggml_type type) {
    switch (type) {
        case LM_GGML_TYPE_Q4_0:
...
static cl_mem lm_ggml_cl_pool_malloc(size_t size, size_t * actual_size) {
...
static void lm_ggml_cl_pool_free(cl_mem mem, size_t size) {
    scoped_spin_lock lock(g_cl_pool_lock);
...
static cl_int lm_ggml_cl_h2d_tensor_2d(cl_command_queue queue, cl_mem dst, size_t offset, const struct lm_ggml_tensor * src, uint64_t i3, uint64_t i2, cl_event* ev) {
    cl_int err;
...
static void lm_ggml_cl_mul_f32(const lm_ggml_tensor * src0, const lm_ggml_tensor * src1, lm_ggml_tensor * dst) {
    LM_GGML_ASSERT(src1->backend == LM_GGML_BACKEND_GPU);
    ...
    cl_mem d_X = lm_ggml_cl_pool_malloc(ne0 * sizeof(float), &x_size); // src0
    cl_mem d_Y = (cl_mem) src1->extra; // src1 is already on device, broadcasted.
    cl_mem d_D = lm_ggml_cl_pool_malloc(ne0 * sizeof(float), &d_size); // dst
    ...
    for (int64_t i03 = 0; i03 < ne03; i03++) {
        for (int64_t i02 = 0; i02 < ne02; i02++) {
            ...
            // copy src0 to device
            CL_CHECK(lm_ggml_cl_h2d_tensor_2d(queue, d_X, i0, src0, i03, i02, &ev));
    ...
    lm_ggml_cl_pool_free(d_X, x_size);
    lm_ggml_cl_pool_free(d_D, d_size);
}

void lm_ggml_cl_mul(const struct lm_ggml_tensor * src0, const struct lm_ggml_tensor * src1, struct lm_ggml_tensor * dst) {
    LM_GGML_ASSERT(src0->type == LM_GGML_TYPE_F32 && src1->type == LM_GGML_TYPE_F32 && dst->type == LM_GGML_TYPE_F32);
    lm_ggml_cl_mul_f32(src0, src1, dst);
}

static void lm_ggml_cl_mul_mat_f32(const lm_ggml_tensor * src0, const lm_ggml_tensor * src1, lm_ggml_tensor * dst) {
    const int64_t ne00 = src0->ne[0];
    const int64_t ne01 = src0->ne[1];
    const int64_t ne02 = src0->ne[2];
    const int64_t ne03 = src0->ne[3];
    ...
    size_t d_size;
    cl_mem d_X;
    if (src0->backend == LM_GGML_BACKEND_GPU) { // NOLINT
        d_X = (cl_mem) src0->extra;
    } else {
        d_X = lm_ggml_cl_pool_malloc(sizeof(float) * x_ne, &x_size);
    }
    cl_mem d_Y = lm_ggml_cl_pool_malloc(sizeof(float) * y_ne, &y_size);
    cl_mem d_D = lm_ggml_cl_pool_malloc(sizeof(float) * d_ne, &d_size);

    for (int64_t i03 = 0; i03 < ne03; i03++) {
        for (int64_t i02 = 0; i02 < ne02; i02++) {
            // copy data to device
            if (src0->backend != LM_GGML_BACKEND_GPU) {
                CL_CHECK(lm_ggml_cl_h2d_tensor_2d(queue, d_X, 0, src0, i03, i02, NULL));
            }
            CL_CHECK(lm_ggml_cl_h2d_tensor_2d(queue, d_Y, 0, src1, i03, i02, NULL));
    ...
    lm_ggml_cl_pool_free(d_Y, y_size);
    lm_ggml_cl_pool_free(d_D, d_size);
}

static void lm_ggml_cl_mul_mat_f16(const lm_ggml_tensor * src0, const lm_ggml_tensor * src1, lm_ggml_tensor * dst, void * wdata, size_t /* wsize */) {
    LM_GGML_ASSERT(fp16_support);
    ...
    const lm_ggml_fp16_t alpha = lm_ggml_fp32_to_fp16(1.0f);
    const lm_ggml_fp16_t beta = lm_ggml_fp32_to_fp16(0.0f);
    const int x_ne = ne01 * ne00;
    const int y_ne = ne11 * ne10;
    const int d_ne = ne11 * ne01;
    ...
    if (src0->backend == LM_GGML_BACKEND_GPU) { // NOLINT
        ...
        d_X = lm_ggml_cl_pool_malloc(sizeof(lm_ggml_fp16_t) * x_ne, &x_size);
    }
    cl_mem d_Y = lm_ggml_cl_pool_malloc(sizeof(lm_ggml_fp16_t) * y_ne, &y_size);
    cl_mem d_D = lm_ggml_cl_pool_malloc(sizeof(lm_ggml_fp16_t) * d_ne, &d_size);
    ...
    for (int64_t i03 = 0; i03 < ne03; i03++) {
        for (int64_t i02 = 0; i02 < ne02; i02++) {
            ...
            lm_ggml_fp16_t * const tmp = (lm_ggml_fp16_t *) wdata + (ne11 * ne10) * (i03 * ne02 + i02);
            char * src1i = (char *) src1->data + i03*nb13 + i02*nb12;
            if (src1_cont_rows) {
                if (src1_cont_cols) {
                    lm_ggml_fp32_to_fp16_row((float *) src1i, tmp, ne10*ne11);
                }
                else {
                    for (int64_t i01 = 0; i01 < ne11; i01++) {
                        lm_ggml_fp32_to_fp16_row((float *) (src1i + i01*nb11), tmp + i01*ne10, ne10);
                    }
                }
            }
            else {
                for (int64_t i01 = 0; i01 < ne11; i01++) {
                    for (int64_t i00 = 0; i00 < ne10; i00++) {
                        ...
                        tmp[i01*ne10 + i00] = lm_ggml_fp32_to_fp16(*(float *) (src1i + i01*nb11 + i00*nb10));
                    }
                }
            }

            // copy src1 to device
            CL_CHECK(clEnqueueWriteBuffer(queue, d_Y, false, 0, sizeof(lm_ggml_fp16_t) * y_ne, tmp, 0, NULL, NULL));
            ...
            CL_CHECK(clEnqueueReadBuffer(queue, d_D, true, 0, sizeof(lm_ggml_fp16_t) * d_ne, tmp, 1, &ev_sgemm, NULL));

            float * d = (float *) ((char *) dst->data + i02*nb2 + i03*nb3);

            lm_ggml_fp16_to_fp32_row(tmp, d, d_ne);
        }
    }
...
static void lm_ggml_cl_mul_mat_q_f32(const lm_ggml_tensor * src0, const lm_ggml_tensor * src1, lm_ggml_tensor * dst) {
    const int64_t ne00 = src0->ne[0];
    const int64_t ne01 = src0->ne[1];
    const int64_t ne02 = src0->ne[2];
    const int64_t ne03 = src0->ne[3];
    ...
    const int y_ne = ne11 * ne10;
    const int d_ne = ne11 * ne01;
    const size_t q_sz = lm_ggml_type_size(type) * x_ne / lm_ggml_blck_size(type);
    ...
    if (src0->backend == LM_GGML_BACKEND_CPU) {
        d_Q = lm_ggml_cl_pool_malloc(q_sz, &q_size);
    }

    cl_kernel* to_fp32_cl = lm_ggml_get_to_fp32_cl(type);
    cl_kernel* dmmv = lm_ggml_get_dequantize_mul_mat_vec_cl(type);
    LM_GGML_ASSERT(to_fp32_cl != nullptr);

    const size_t global_denom = lm_ggml_cl_global_denom(type);
    const size_t local = lm_ggml_cl_local_size(type);
    ...
    for (int64_t i03 = 0; i03 < ne03; i03++) {
        for (int64_t i02 = 0; i02 < ne02; i02++) {
            ...
            if (src0->backend == LM_GGML_BACKEND_CPU) {
                events.emplace_back();
                CL_CHECK(lm_ggml_cl_h2d_tensor_2d(queue, d_Q, 0, src0, i03, i02, events.data() + ev_idx++));
            ...
            if (mul_mat_vec) { // specialized dequantize_mul_mat_vec kernel
                // copy src1 to device
                events.emplace_back();
                CL_CHECK(lm_ggml_cl_h2d_tensor_2d(queue, d_Y, 0, src1, i03, i02, events.data() + ev_idx++));
    ...
    lm_ggml_cl_pool_free(d_D, d_size);
    if (src0->backend == LM_GGML_BACKEND_CPU) {
        lm_ggml_cl_pool_free(d_Q, q_size);
    }
}
...
bool lm_ggml_cl_mul_mat_use_f16(const struct lm_ggml_tensor * src0, const struct lm_ggml_tensor * src1, struct lm_ggml_tensor * /* dst */) {
    // If device doesn't support FP16
    if (!fp16_support) {
        return false;
    }

    size_t src0_sz = lm_ggml_nbytes(src0);
    size_t src1_sz = lm_ggml_nbytes(src1);

    // mul_mat_q: src0 is converted to fp32 on device
    size_t mul_mat_q_transfer = src0_sz + src1_sz;

    // mul_mat_f16: src1 is converted to fp16 on cpu
    size_t mul_mat_f16_transfer = src0_sz + sizeof(lm_ggml_fp16_t) * lm_ggml_nelements(src1);
...
void lm_ggml_cl_mul_mat(const struct lm_ggml_tensor * src0, const struct lm_ggml_tensor * src1, struct lm_ggml_tensor * dst, void * wdata, size_t wsize) {
    LM_GGML_ASSERT(lm_ggml_cl_can_mul_mat(src0, src1, dst));

    if (src0->type == LM_GGML_TYPE_F32) {
        lm_ggml_cl_mul_mat_f32(src0, src1, dst);
    }
    else if (src0->type == LM_GGML_TYPE_F16) {
        if (lm_ggml_cl_mul_mat_use_f16(src0, src1, dst)) {
            lm_ggml_cl_mul_mat_f16(src0, src1, dst, wdata, wsize);
        }
        else {
            lm_ggml_cl_mul_mat_q_f32(src0, src1, dst);
        }
    }
    else if (lm_ggml_is_quantized(src0->type)) {
        lm_ggml_cl_mul_mat_q_f32(src0, src1, dst);
    }
    else {
        LM_GGML_ASSERT(false);
...
size_t lm_ggml_cl_mul_mat_get_wsize(const struct lm_ggml_tensor * src0, const struct lm_ggml_tensor * src1, struct lm_ggml_tensor * dst) {
    if (lm_ggml_cl_mul_mat_use_f16(src0, src1, dst)) {
        return lm_ggml_nelements(src1) * sizeof(lm_ggml_fp16_t);
    }
    return 0;
}

void lm_ggml_cl_transform_tensor(void * data, lm_ggml_tensor * tensor) {
    ...
    size_t q_size;
    cl_mem dst = lm_ggml_cl_pool_malloc(q_sz, &q_size);
    ...
    for (int64_t i3 = 0; i3 < ne3; i3++) {
        for (int64_t i2 = 0; i2 < ne2; i2++) {
            int i = i3*ne2 + i2;
            CL_CHECK(lm_ggml_cl_h2d_tensor_2d(queue, dst, i*ne0*ne1, tensor, i3, i2, NULL));
        }
    }
